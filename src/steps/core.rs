...
impl Step for LinkFiles {
    fn execute(&self, context: &Context) -> Result<(), StepError> {
        for file in context.config.link_files.iter() {
            ...
            let file_exists = exec_remote_file_exists(
                &context.config.host,
                &shared_file_path,
                FSResourceType::File,
            )?;
            ...
            if !file_exists {
                let error_msg = format!(
                    "Could not create symlink for file {} because it doesn't exist",
                    file
                );
...
impl Step for LinkDirs {
    fn execute(&self, context: &Context) -> Result<(), StepError> {
        for dir in context.config.link_dirs.iter() {
            ...
            let dir_exists = exec_remote_file_exists(
                &context.config.host,
                &shared_dir_path,
            ...
            if !dir_exists {
                let error_msg = format!(
                    "Could not create symlink for dir {} because it doesn't exist",
                    dir
                );
                return Err(StepError::Critical(error_msg));
            }
...
impl Step for SymlinkCurrent {
    fn execute(&self, context: &Context) -> Result<(), StepError> {
        ...
        let current_symlink_exist = exec_remote_file_exists(
            &context.config.host,
            &current_symlink_path,
            FSResourceType::Symlink,
        )?;
        ...
        let create_current_symlink_cmd = format!(
            "ln -s {} {}",
...
impl Step for CleanUpReleases {
    fn execute(&self, context: &Context) -> Result<(), StepError> {
        let mut releases = exec_remote_fetch_sorted_filenames_in_dir(
            &context.config.host,
            &context.releases_path,
            SortOrder::Asc,
